...
namespace The_Long_Dark_Save_Editor_2.Game_data
{
    ...
    public class GearItemSaveDataProxy : BindableBase
    {
        ...
        public static GearItemSaveDataProxy Create(DynamicSerializable<GlobalSaveGameFormat> Global)
        {
            ...
            item.m_HoursPlayed = Global.Obj.TimeOfDay.m_HoursPlayedNotPausedProxy;
            var r = new Random();
            var id = r.Next();
            while (Global.Obj.Inventory.Items.Any(i => i.Gear.m_InstanceIDProxy == id))
                id = r.Next();
